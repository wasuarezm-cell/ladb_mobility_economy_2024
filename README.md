...
👉 **[Open Notebook in Google Colab](https://www.google.com/search?q=https://colab.research.google.com/drive/1AeRWISu0kwJPH94dVlmNUQ9JW3WEbvy_)**

✔ How to reproduce this analysis?

To run the analysis from scratch and get the same results, follow these steps:

1. **Open the main file**: Launch the `S5 ladb_mobility_economy_project_student.ipynb` notebook in Google Colab using the link above, or clone it to use in your local Jupyter environment.
2. **Load the Datasets**: Data loading is programmed directly in "Step 1" of the notebook using `pandas`. It uses the internal paths `/datasets/tomtom_traffic.csv` and `/datasets/oecd_city_economy.csv`. If you are running this locally, make sure to have the CSV files located in a `datasets` folder in the same directory as your environment.
3. **Sequential Execution**: Run the cells in descending order. The notebook is designed to follow a logical flow:
